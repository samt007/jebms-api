package com.jebms.comm.springjdbc;
...
public class DevJdbcTemplate{
     ...
     * Logger
     */
    protected static final Logger LOGGER = LoggerFactory.getLogger(DevJdbcTemplate.class);
	...
	public <T> List<T> queryForList(String sql,Map<String,Object> paramMap,RowMapper<T> rowMapper) throws Exception{
		LOGGER.info("queryForList sql:"+sql);
	...
	public <T> T queryForObject(String sql,Map<String,Object> paramMap,RowMapper<T> rowMapper) throws Exception{
		LOGGER.info("queryForObject sql:"+sql);
	...
	public List<Object[]> queryForObjArrList(String sql,Map<String,Object> paramMap) throws Exception{
		LOGGER.info("queryForObjArrList sql:"+sql);
	...
	public List<Map<String, Object>> queryForList(String sql,Map<String,Object> paramMap) throws Exception{
		LOGGER.info("queryForList sql:"+sql);
		return getNamedParameterJdbcOperations().queryForList(sql, paramMap);
	}
		...
		try{
	        ...
	        for(SqlParameter sqlParam:declaredParameters){
	        	if(inParamMap.containsKey(sqlParam.getName())){
	        		paramLog.append("[inParam:"+sqlParam.getName()+",value:"+inParamMap.get(sqlParam.getName())+"]");
	            	cs.setObject(sqlParam.getName(), inParamMap.get(sqlParam.getName()));
	        	}
	        	if(outParamMap.containsKey(sqlParam.getName())){
	        		paramLog.append("[outParam:"+sqlParam.getName()+"]");
			...
			con = null;
			LOGGER.info("execute ex LOG:"+ex.toString());
	...
	/**
	 * 执行PLSQL处理，输出ResultEntity。这里完全可以输入输出参数的处理。
	 * <br/>输出参数默认是自动匹配的(自动匹配ResultEntity)，但是编写PLSQL的时候必须要注意命名规则。
	 * <br/>
	 * @param sql 处理语句
	 * @param inParamMap 输入参数
	...
	public ResultEntity executeForResultEntity(String sql,Map<String,Object> inParamMap) throws Exception{
		Map<String,Integer> outParamMap=new HashMap<String,Integer>();
		ResultEntity ret=ResultInfo.error();
		checkExecuteParam=false;
		outParamMap.put(ResultEntity.CODE, Types.INTEGER);
		outParamMap.put(ResultEntity.MESSAGE, Types.VARCHAR);
		outParamMap.put(ResultEntity.DESCRIPTION, Types.VARCHAR);
		outParamMap.put(ResultEntity.PARAM1, Types.VARCHAR);
		outParamMap.put(ResultEntity.PARAM2, Types.VARCHAR);
		outParamMap.put(ResultEntity.PARAM3, Types.VARCHAR);
		outParamMap.put(ResultEntity.PARAM4, Types.VARCHAR);
		outParamMap.put(ResultEntity.PARAM5, Types.VARCHAR);
		Map<String,Object> retValueMap=execute(sql,inParamMap,outParamMap);
		for(String key:retValueMap.keySet()){
			if(key.equals(ResultEntity.CODE)) {
				ret.setCode(retValueMap.get(key).toString());
				continue;
			}
			if(key.equals(ResultEntity.MESSAGE)) {
				ret.setMessage((String) retValueMap.get(key));
				continue;
			}
			if(key.equals(ResultEntity.DESCRIPTION)) {
				ret.setDescription((String) retValueMap.get(key));
				continue;
			}
			if(key.equals(ResultEntity.PARAM1)) {
				ret.setParam1((String) retValueMap.get(key));
				continue;
			}
			if(key.equals(ResultEntity.PARAM2)) {
				ret.setParam2((String) retValueMap.get(key));
				continue;
			}
			if(key.equals(ResultEntity.PARAM3)) {
				ret.setParam3((String) retValueMap.get(key));
				continue;
			}
			if(key.equals(ResultEntity.PARAM4)) {
				ret.setParam4((String) retValueMap.get(key));
				continue;
			}
			if(key.equals(ResultEntity.PARAM5)) {
