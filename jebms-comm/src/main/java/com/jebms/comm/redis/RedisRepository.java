...
public class RedisRepository {
    ...
    /**
     * 一次性更新数组 的   缓存过期时间的  ，不用多次连接，节省开销
     *
     * @param keys   redis主键数组
     * @param time   过期时间
     */
    public void updateExpire(final String[] keys, final long time) {
        redisTemplate.execute((RedisCallback<Long>) connection -> {
            RedisSerializer<String> serializer = getRedisSerializer();
            for (int i = 0; i < keys.length; i++) {
                byte[] bKeys = serializer.serialize(keys[i]);
                connection.expire(bKeys, time);
                LOGGER.info("[redisTemplate redis]更新 缓存  url: {} ========更新缓存时间为:{}秒", keys[i], time);
            }
            return 1L;
        });
    }
     ...
     *
     * @param key  the key
     * @param time the time 毫秒
     */
    public void updateExpire(final String key, final long time) {
    ...
    public Long getExpire(final String key) {
    	...
    	LOGGER.info("[redisTemplate redis]查询 缓存  url: {} 的剩余有效时间: {}", key, expireTime);
    	return expireTime;
    }
