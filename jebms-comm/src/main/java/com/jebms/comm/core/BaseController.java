...
public abstract class BaseController {
    ...
    protected AuthUser authUser;//当前的认证用户
    ...
    public void setReqAndRes(HttpServletRequest request, HttpServletResponse response) throws UnsupportedEncodingException{ 
        ...
        this.authUser = UserUtil.getCurrentUser();
        // System.out.println("用户ID:"+authUser.getId()+",登录ID:"+authUser.getLoginId()+",登录语言:"+authUser.getLanguage());
    }
	...
	private Integer toInt(String value, Integer defaultValue) {
			if (TypeConverter.isEmpty(value))
				return defaultValue;
			return TypeConverter.str2Int(value.trim());
	}
	/**
	...
	private Long toLong(String value, Long defaultValue) {
		if (TypeConverter.isEmpty(value))
			return defaultValue;
		return TypeConverter.str2Long(value.trim());
	}
	/**
	...
	private Boolean toBoolean(String value, Boolean defaultValue) {
		...
		return TypeConverter.str2Boolean(value.trim());
	}
	...
	private Date toDate(String value, Date defaultValue){
		...
		if (TypeConverter.isEmpty(value))
			return defaultValue;
		try {
			d= TypeConverter.str2uDate(value.trim());
	...
	public <T> T getEntity(Class<T> entityClass) {
		return (T)Injector.injectEntity(entityClass, request, false);
	}
	
	public <T> T getEntity(Class<T> entityClass, boolean skipConvertError) {
		return (T)Injector.injectEntity(entityClass, request, skipConvertError);
	}
	
	public <T> T getEntity(Class<T> entityClass, String beanName) {
		return (T)Injector.injectEntity(entityClass,beanName, request, false);
	}
	
	public <T> T getEntity(Class<T> entityClass, String beanName, boolean skipConvertError) {
		return (T)Injector.injectEntity(entityClass, beanName, request, skipConvertError);
	}
	
	
	@SuppressWarnings("unchecked")
	public <T> T getEntity(Object entity) {
		return (T)Injector.injectEntity(entity, request, false);
	}
	
	@SuppressWarnings("unchecked")
	public <T> T getEntity(Object entity, boolean skipConvertError) {
		return (T)Injector.injectEntity(entity, request, skipConvertError);
	}
	
	@SuppressWarnings("unchecked")
	public <T> T getEntity(Object entity, String beanName) {
		return (T)Injector.injectEntity(entity,beanName, request, false);
	}
	
	@SuppressWarnings("unchecked")
	public <T> T getEntity(Object entity, String beanName, boolean skipConvertError) {
		return (T)Injector.injectEntity(entity, beanName, request, skipConvertError);
	}
