...
public class TypeConverter {
     ...
     */
    public static boolean isNotEmpty(Object x) {
        return !isEmpty(x);
    }
    ...
    public static Long str2Long(String x){
		return isEmpty(x)?null:Long.parseLong(x);
	}
    public static Integer str2Int(String x){
		return isEmpty(x)?null:Integer.parseInt(x);
	}
    public static Double str2Double(String x){
		return isEmpty(x)?null:Double.parseDouble(x);
	}
    ...
    public static Clob str2sClob(String x) throws SerialException, SQLException{
		return isEmpty(x)?null:(Clob)new javax.sql.rowset.serial.SerialClob(x.toCharArray());
